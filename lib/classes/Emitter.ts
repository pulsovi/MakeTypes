...
export default class Emitter {
  ...
  public emit(root: any, rootName: string): void {
    ...
    if (!(rootShape instanceof RecordShape)) {
      ...
      const roots = new Set<RecordShape>();
      getReferencedRecordShapes(this, roots, rootShape);
      let rootArray = new Array<RecordShape>();
      ...
      if (rootShape.type !== BaseShape.NEVER) this.proxies.tab(2).writeln(`return s;`);
      this.proxies.tab(1).writeln(`}`);
  ...
  private _emitRootRecordShape(name: string, rootShape: RecordShape) {
   ...
   */
  public registerRecordShape(s: RecordShape): RecordShape {
